...
- Navbar collapses to hamburger below `lg: 1024px`. Tablet landscape (1024px+) shows the **same full nav as desktop, compacted to fit** — not a separate hamburger tier. A first attempt at fixing tablet-landscape overflow raised this breakpoint to `xl: 1280px` (hiding the full nav behind a hamburger through the whole 1024–1279px range); the user explicitly rejected that and asked for the real nav, sized down instead. The working pattern: size everything compact by default (this is what renders from `lg:` up), then use `xl:` overrides to restore the roomier desktop sizing at 1280px+. Don't default to "just raise the breakpoint and hide it" when a tablet-range overflow shows up — compacting in place is usually what's actually wanted.
- Navbar's floating `sticky top-5` gap only applies at `sm:` (640px) and up — below that (true mobile only), the header sits flush (`top-0`) with no gap. This is a *different* breakpoint than the hamburger/full-nav switch (`lg:`, 1024px) — a first attempt paired the two together, which gave tablet portrait (768px) the flush mobile treatment too; the user narrowed it back to mobile-only via `/recover`. Don't assume two responsive concerns (here: "should the gap be flush" and "should the nav be hamburger-vs-full") share the same breakpoint just because they're both about the navbar — check each independently.
...
- **`html`/`body` carry `overflow-x-hidden` (`globals.css` `@layer base`) as a defensive, document-level guard against page-level horizontal scroll** — added after a real bug where `CompanyLogosStrip`'s internal `overflow-x: auto` scroll row leaked into `document.documentElement.scrollWidth` roughly 1.3 seconds after mount (confirmed via direct mouse-wheel scroll testing that a real user could pan the whole page sideways into blank space), despite the row itself being correctly scoped — root cause traced to a genuine browser-level scrollable-overflow quirk, not a fixable CSS property on the component itself (`mask-image` removal didn't help; by the time the leak appeared every animated child had already fully settled to `transform: none`/`opacity: 1`). This guard does **not** break any component's own intentional internal horizontal scrolling (e.g. this same logos row) — clipping at the document root has no effect on a descendant's independent `overflow-x: auto`/`scrollLeft` behavior. If a future feature ever needs the page itself to scroll horizontally (unlikely), this rule would need to be revisited explicitly.
- **Standing rule from Feature 09's responsive audit: every feature must be checked at every standard screen size before being considered done** — not just one desktop width and one mobile width. At minimum: a laptop viewport (e.g. 1366×768 — note the *height*, not just width, matters for tall interactive content), tablet portrait (768×1024) and landscape (1024×768), and mobile (390×844). When checking for horizontal overflow, measure `document.documentElement.scrollWidth` vs `clientWidth` directly **at more than one point in time after load** — a one-shot check can land in a transient clean window and miss a real, timing-dependent leak (this happened during this exact audit). Don't assume a similar-looking number is the same "known baseline" as last time without re-confirming which element is actually responsible by elimination (hide-and-remeasure), not by guessing from a screenshot
