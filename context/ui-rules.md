...
- **Hero simulators show full, functional controls on mobile** — flipped from the original "simplified/hidden by default" plan. The Event Loop simulator (Feature 09) showed full controls on mobile first, as a CodeRabbit-driven, single-feature exception; when Feature 10 (React Rendering) came up, the user confirmed this should be the standing default going forward for all hero simulators (10–12 included), not re-litigated per feature. Build new simulators with controls visible and tappable on mobile by default.
