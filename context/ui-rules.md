...
- Navbar's floating `sticky top-5` gap only applies at `lg:` and up (paired with the hamburger/full-nav breakpoint above) — below that, the header sits flush (`top-0`) with no gap. The floating-pill-with-breathing-room look is a desktop bonus; small screens get a plain flush header bar.
