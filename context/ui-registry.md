...
| `PlatformGraph` file split | Split across 4 files once the rendering-bug fixes below pushed it past `code-standards.md`'s 200-line limit (was 273 lines): `graphData.ts` (topic/collection data, the bezier point-sampler, `linkPath`, `cardEntryY`, the viewBox-aspect constants/helper), `useGraphMeasurements.ts` (a dedicated hook owning all the refs + the measurement `useLayoutEffect`), `CollectionsCard.tsx` (the small repeated numbered-list card, used by both the desktop graph and the mobile fallback), and `PlatformGraph.tsx` itself (183 lines — orchestration + JSX only) |
...
| `PlatformGraph` motion | Each `<motion.path>` draws itself in via `pathLength` (0 → 1, the documented SVG path-draw technique already named in `ui-rules.md`'s Motion section), propagated from a single parent `whileInView` trigger via Framer Motion `variants` (`fadeInVariants`/`pathVariants`) — **not** independent per-path `whileInView` triggers, consistent with this codebase's "observe the parent only" lesson from `AnimatedHeading`'s zero-area bug. The topmost curve on each side renders with an `xp`→`streak` (amber→orange) linear gradient (`#ff-graph-highlight`) instead of plain `stroke-border-muted`, as a featured-path accent echoing the reference image's own yellow-orange highlighted curves — reusing the gamification XP/streak tokens purely for hue, same precedent as `HeroSimulatorPreview`/`StepSection`. **Continuous motion was added back** after the user said the graph felt "very static": a soft pulsing glow (`opacity`/`scale` loop) behind the center card, a small `Sparkles` icon doodle floating/rotating above it, and a small dot continuously traveling along each of the 2 highlighted curves — computed via `sampleLink()` in `graphData.ts` (the same cubic-bezier control points as the visible curve, sampled at several `t` steps into `cx`/`cy` keyframe arrays) so the dot visually follows the curve's actual shape rather than cutting a straight line across it |
