...
| Property | Detail |
| --- | --- |
| `PlatformGraph` (`feature-highlights/PlatformGraph.tsx`) | New hero visual, rendered between the section subtext and the card grid — added per explicit user request for "a graph where Frontend Forever in center and other Features connecting through graph lines that are glowing or travelling to it." A center `bg-surface` circle ("FF" + a soft `blur-xl` accent glow) with 4 nodes at fixed cardinal positions (top/right/bottom/left, `NODE_POSITIONS` in percent, mapped 1:1 to `FEATURES` order so node color/icon always matches the card below it), connected by an SVG overlay: per line, a wide low-opacity `<line>` (glow) + a thin bright `<line>` on top (no `<feGaussianBlur>` filter — kept to plain stroke-width/opacity layering, cheaper and consistent with how this codebase does glow elsewhere via blurred `div`s, not SVG filters) + a `motion.circle` animating `cx`/`cy` from the outer node to center on an infinite loop, staggered per line by `index * 0.35s`. One `LINE_THEME: Record<FeatureThemeColor, { stroke, fill, node }>` map drives line/dot/node color per feature. Uses Tailwind v4's auto-generated `stroke-*`/`fill-*` utilities against this project's custom `--color-accent`/`info`/`premium`/`success` theme variables — confirmed via screenshot that these resolve correctly the same way `bg-*`/`text-*` already do, no separate SVG color setup needed. The whole graph has one `whileInView` scale/opacity entrance; the per-line traveling dots loop forever afterward — **a deliberate, explicitly-requested exception** to this same feature's "no continuous motion" decision below, which is scoped to the 4 cards only, not this centerpiece |
