...
**The Next.js collection is complete: 99/99 questions**, and with it, **the entire 299-question interview-prep content run across all three collections (101 JavaScript / 99 React / 99 Next.js) is complete.** This branch's own 12 hand-authored diagram components (10 authored across the Next.js batches, plus the 2 shared pilot diagrams and one duplicate of `ReactServerComponentsDiagram`, reconciled to a single copy on merge) join the 8 from the JavaScript collection and 16 from the React collection. All three collection branches (`content/ff-javascript-questions`, `content/ff-react-questions`, `content/ff-nextjs-questions`) are now reconciled into one `develop`-bound history via this merge — no unresolved conflicts, `npx tsc --noEmit` clean, and `npx tsx scripts/seed.ts` succeeds seeding all 299 real questions plus roadmap/challenge/concept data unchanged.

**Post-merge audit + FF 75 curation (2026-07-16/17):** a full-collection review after the 3-way merge (`fix/ff-nextjs-broken-related-links`, PR #81) found and fixed 2 broken cross-collection "Related" links in the Next.js collection pointing at stale pre-final React slugs — everything else (question/`orderIndex` counts, slug uniqueness, all 419 internal links, all `DIAGRAM_BY_SLUG` entries) checked out clean. FF 75 was then curated (`fix/ff-75-curation-and-collection-order`, PR #82): `isFf75: true` set on 75 of the 299 questions — 36 JavaScript / 26 React / 13 Next.js, no fixed per-collection quota, ranked by cross-referencing GreatFrontEnd's current top-50 JS/React lists and 3 Next.js "most-asked" sources against real interview frequency rather than by even/weighted split. The same review also caught and fixed a real bug unrelated to content: the `ff-75` virtual collection's query sorted alphabetically by the `collection` column (`"ff-nextjs"` < `"ff-react"`), putting Next.js questions before React ones — fixed to sort by the IA's actual intended order instead. See `progress-tracker.md` → "Content Initiative — Interview Prep FF Collections (299 Questions + FF 75 Curation)" for the full build detail.
