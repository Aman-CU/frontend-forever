...
**Last completed:** 03 Hero Section (Static) — UI built and verified (typecheck + lint clean, pixel-checked against `designs/hero-section-1-event-loop.png` in light, dark, and mobile, see Notes below). Built on `feature/03-hero-section` off `develop`. Not yet committed/pushed/PR'd.
**Currently building:** Nothing in progress.
**Next:** Commit + PR `feature/03-hero-section` → `develop`, then start 04 Company Logos Strip
