...
**Related:** How do you handle forms in React? · What is the difference between state and props? · [Forms: Controlled vs. Uncontrolled](/learn/react/controlled-vs-uncontrolled-forms) (Learn concept)`,
  ...
  },
  {
    ...
    answer: `\`useMemo(fn, deps)\` caches the return value of an expensive computation across renders, only recomputing it when one of the values in \`deps\` changes. **As of React Compiler (shipped in React 20), this is worth answering carefully** — the compiler now auto-memoizes most component output, so manual \`useMemo\` has shifted from routine practice to a targeted escape hatch.
