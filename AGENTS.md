...
**A route 404s in `next dev` even though the file/content is correct — Turbopack route-discovery quirk, not a bug.** Confirmed (2026-07-22, `context/progress-tracker.md` → "Fix — FF Collections Completion Tracking"): a nested dynamic route (`/interview-prep/playbook/[playbookSlug]` and `/interview-prep/playbook/[playbookSlug]/[chapterSlug]`) 404'd on every request — Turbopack's dev server logged `Compiling /_not-found/page`, never even attempting to compile the real route. A full `.next` wipe + cold restart did **not** fix it. What did: a no-op save (add a blank line, save, remove it, save again — byte-identical to the original) of that route's `page.tsx` while the dev server was running. That single save fixed the route for every param value under it (all 6 playbooks, not just the one touched), and the fix survived reverting the edit. Recurred later the same day on a fresh boot — that time it hit `/api/auth/[...all]` itself, which broke session checks (`/api/auth/get-session` 404 → client renders logged-out even with a valid cookie, looks like a flickering/broken login). **Dev-only** — a production build (`next build`) compiles every route ahead of time, so this failure mode cannot occur in production regardless of how it manifests in dev.

Tested switching to `next dev --webpack` as a permanent fix (same day): it does not 404 silently, but cold-compile per route was consistently worse (most routes exceeded 60s on a cleared cache, only one finished under a minute) than Turbopack's already-slow worst case (~95s seen once, usually well under that) — not a worthwhile trade. Instead: **run `npm run warmup` right after `npm run dev` reports "Ready"** — `scripts/warmup-dev.mjs` hits one real URL per known route file (including `/api/auth/get-session`) so everything is compiled before you start clicking around, instead of discovering 404s one at a time. Update that file's `ROUTES` list when a new top-level route or dynamic segment is added. If a route still 404s after warming up (e.g. one added after the list was last updated): open its `page.tsx`/`route.ts` and save it once — don't just restart the dev server, that alone doesn't fix this specific failure mode.
