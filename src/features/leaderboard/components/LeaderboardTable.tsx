...
export function LeaderboardTable({
  ...
  return (
    ...
    >
      {/* min-w on both the header and data rows below keeps every column
          (including "Done") from clipping under overflow-hidden on narrow
          viewports — the fixed 16rem of columns + gaps + padding already
          exceeds a 320px screen, so this wrapper scrolls horizontally
          instead of silently cutting off the rightmost columns. */}
      <div className="overflow-x-auto">
        <div
          role="row"
          className="grid min-w-[30rem] grid-cols-[3rem_minmax(0,1fr)_5rem_4rem_4rem] items-center gap-3 border-b border-border-light px-4 py-2.5 text-xs font-semibold uppercase tracking-wide text-text-muted sm:px-5"
        >
        ...
        <div className="divide-y divide-border-light">
          {entries.map((entry) => (
            <div
              ...
              role="row"
              className={cn(
                "grid min-w-[30rem] grid-cols-[3rem_minmax(0,1fr)_5rem_4rem_4rem] items-center gap-3 px-4 py-3 sm:px-5",
