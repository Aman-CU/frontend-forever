...
import { RENDER_SCENARIOS } from "../data/scenarios";
import type { RenderFrame, RenderScenario } from "../types";

const BASE_INTERVAL_MS = 1800;
const SCENARIO_OPTIONS: ScenarioOption[] = RENDER_SCENARIOS.map((s) => ({
  id: s.id,
  label: s.label,
}));

type UseReactRenderingSimulatorOptions = {
...
export function useReactRenderingSimulator(
  { onReachedEnd }: UseReactRenderingSimulatorOptions = {},
): UseReactRenderingSimulatorResult {
  const [activeScenarioId, setActiveScenarioId] = useState(RENDER_SCENARIOS[0].id);
  ...
  const scenario =
    RENDER_SCENARIOS.find((s) => s.id === activeScenarioId) ?? RENDER_SCENARIOS[0];
