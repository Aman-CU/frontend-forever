...
// Hand-inlined from devicon.dev (MIT licensed) — fill is the real TS brand
// color via --color-ts-badge-bg (globals.css), the same token
// categoryMeta.ts's TS letter badge already uses elsewhere in the app, kept
// consistent rather than hardcoding devicon's own slightly different blue.
export function TypescriptIcon({ className }: Props) {
  return (
    <svg viewBox="0 0 128 128" aria-hidden="true" className={cn("h-5 w-5", className)}>
      <path
        fill="var(--color-ts-badge-bg)"
